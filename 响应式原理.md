...
export function observe(value){
   //...省略一些检验代码，必须是对象并且可以延展的，已经响应化了就直接返回等等
   return new Observer(value)
}
```
并不是所有值都可以响应化，需要加一些过滤条件，符合条件的就`new Observer`，对数组或者对象进行响应式定义。
...
```javascript
  class Observer {
    ...
    if (Array.isArray(value)) {
      ...
      //循环遍历数组，数组每一项的对象值进行响应式化。
      this.observeArray(value)
    } else {
      // 对对象的每一个可以值进行setter，getter设置
      this.walk(value)
    }
    }
  }
...
```
可以看出这个类，主要对数组的`push`等方法进行重写，对对象调用`defineReactive`，即对对象进行`setter``getter`定义;
...
## 依赖收集
一个属性值，可能有多个观察者，一个观察者可能有多个属性依赖
![响应式原理](./depAndWatcher.png)
### Dep
`Dep`位于`src/core/observer/dep.js`下,是一个收集观察者订阅，以及通知观察更新的一个容器，总而言之，是一个用来管理观察者的类
```javascript
let uid = 0
...
### Watch
`Watcher`这个类位于`src/core/observer/watcher.js`，Watch类就是观察者，产生订阅，以及更新视图方法都在这里
简化一下代码
```javascript
  class Watcher{
    constructor(){
